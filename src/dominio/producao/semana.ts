...
import { inicioDaSemana, iso, DIAS_UTEIS, type Pedido } from './tipos'

/* ---------- por que "dia" e string, e nao Date ---------------------------
   planejado_em e fechado_em sao DATE no banco, sem hora e sem fuso. Virar
   Date e voltar e onde nascem os erros de um dia para menos: `new Date(
   '2026-09-21')` e meia-noite em UTC, que em Goiania ainda e dia 20. Aqui a
   data anda como 'AAAA-MM-DD' o tempo todo, e so vira Date dentro de
   `emMeioDia`, que crava 12h para nenhum horario de verao empurrar o dia. */

export type DiaISO = string

export function emMeioDia(dia: DiaISO): Date {
  const [a, m, d] = dia.split('-').map(Number)
  return new Date(a, m - 1, d, 12, 0, 0, 0)
}

/** A segunda-feira da semana a que este dia pertence, em ISO. */
export function segundaDe(dia: DiaISO): DiaISO {
  return iso(inicioDaSemana(emMeioDia(dia)))
}

/** Quantos dias de distancia, de a para b. Negativo quer dizer que b e antes. */
function distancia(a: DiaISO, b: DiaISO): number {
...
export function indiceNaSemana(dia: DiaISO): number {
...
export function encaixarNaSemana(dia: DiaISO): DiaISO {
  const i = indiceNaSemana(dia)
  if (i < DIAS_UTEIS) return dia
...
export function semanaTerminou(dia: DiaISO, hoje: DiaISO): boolean {
...
export type Origem =
  /** finalizado: ancorado no dia em que ficou pronto, e nao anda mais */
  | 'finalizado'
  /** entrou no sistema e ninguem planejou ainda */
  | 'novo'
  /** a semana dele acabou sem finalizar: caiu na segunda desta semana */
  | 'arrastado'
  /** esta no dia em que alguem planejou */
  | 'planejado'

export type Colocacao = { dia: DiaISO; origem: Origem }
...
export function colocar(p: Pedido, hoje: DiaISO): Colocacao {
  ...
  if (p.fechadoEm) return { dia: encaixarNaSemana(p.fechadoEm), origem: 'finalizado' }
  ...
  if (!p.planejadoEm) return { dia: segundaDeHoje, origem: 'novo' }
  ...
  /* 3. A SEMANA DELE ACABOU E ELE NAO FINALIZOU: segunda desta semana.
        ...
        ou de seis meses. */
  if (semanaTerminou(p.planejadoEm, hoje)) return { dia: segundaDeHoje, origem: 'arrastado' }
...
export type DiaDoPainel = {
  dia: DiaISO
  indice: number
  pedidos: Pedido[]
  /** quantos daquele dia vieram empurrados de uma semana que acabou */
  arrastados: number
  pecas: number
}

export type SemanaDoPainel = {
  segunda: DiaISO
  dias: DiaDoPainel[]
  /** todos os pedidos que a semana mostra, na ordem das colunas */
  pedidos: Pedido[]
  /** os que cairam aqui porque a semana deles acabou, ou porque sao novos */
  pendencia: Pedido[]
  /** a colocacao de cada pedido, por id, para a linha saber como se desenhar */
  onde: Map<string, Colocacao>
}
...
export function montarSemana(
  ...
  const onde = new Map<string, Colocacao>()
  const dias: DiaDoPainel[] = []
  ...
  for (const p of todos) {
    ...
    if (c.origem === 'arrastado' || c.origem === 'novo') coluna.arrastados++
  }
...
export function aoVirarODia(quando: (hojeNovo: DiaISO) => void): () => void {
  ...
  function agendar() {
    ...
    const meiaNoite = new Date(
      ...
      0,
      0,
      30, // trinta segundos depois, para nenhum arredondamento cair em 23:59:59
      0,
    )
    id = setTimeout(() => {
      quando(iso(new Date()))
...
export function hojeISO(): DiaISO {
  return iso(new Date())
}
